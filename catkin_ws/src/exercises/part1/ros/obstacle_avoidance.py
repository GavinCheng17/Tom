...
def braitenberg(front, front_left, front_right, left, right):
  ...
  k2 = -2 #0.75/2.25
  k3 = -4 #1.0/2.25
  vl = -12 - (k1*front + k2*front_right + k3*right) 
  vr = -12 - (k1*front + k2*front_left + k3*left)
  #vl = (k1*front + k2*front_left + k3*left) 
  #vr = (k1*front + k2*front_right + k3*right)  
  #vl = np.tanh(vl)
  #vr = np.tanh(vr)
...
def rule_based(front, front_left, front_right, left, right):
  ...
  if front < 0.3 and front_right < 0.3 and front_left < 0.3:
    u = -0.2
    w = 0.05
  elif front < 0.3 and front_right > 0.3 and front_left < 0.3:
    u = 0
    w = -0.2
  elif front < 0.3 and front_left > 0.3 and front_right < 0.3:
    u = 0
    w = 0.2
  elif front > 1.0 and front_right < front_left:
    w = w + 0.3
  elif front > 1.0 and  front_left < front_right:
    w = w - 0.3

  if left > right and left > 1.2:
    w = w + 0.25
  elif left < right and right > 1.2:
    w = w - 0.25
  #  u 
  #else: 
