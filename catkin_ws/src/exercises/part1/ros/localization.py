...
def braitenberg(front, front_left, front_right, left, right):
  ...
  u =  C1*((vl+vr)) # [m/s]
  w = C2*((vr-vl))  # [rad/s] going counter-clockwise.
...
class Particle(object):
  ...
  def __init__(self):
    ...
    not_val = True
    while (not_val):
      self._pose[X] = 0.8*np.random.randn() - 1
      self._pose[Y] = 0.8*np.random.randn() - 1
      self._pose[YAW] = np.random.uniform(0, 2*np.pi, size=1)
      #print(self._pose)
      if (self.is_valid()):
	not_val = False
  ...
  def is_valid(self):
    ...
    c3 = False
    valid = False
    if(self._pose[X] <= WALL_OFFSET) and (self._pose[X] >= -WALL_OFFSET):
      c1 = True
    if(self._pose[Y] <= WALL_OFFSET) and (self._pose[Y] >= -WALL_OFFSET):
      c2 = True
    r = self._pose[:2] - CYLINDER_POSITION
    r = r.dot(r)
    r = np.sqrt(r)
  ...
  def move(self, delta_pose):
    std_u = np.random.randn()*delta_pose[X]/10
    std_w = np.random.randn()*delta_pose[YAW]/10
    udt = delta_pose[X] + std_u
    wdt = delta_pose[YAW] + std_w

    self._pose[X] += udt*np.cos(self._pose[YAW])
    self._pose[Y] += udt*np.sin(self._pose[YAW])
    self._pose[YAW] += wdt

    o = np.random.uniform(0, 1000, size=1)
    if o < 20:
      not_val = True
      while (not_val):
        self._pose[X] = 0.8*np.random.uniform(-WALL_OFFSET, WALL_OFFSET, size=1)
        self._pose[Y] = 0.8*np.random.uniform(-WALL_OFFSET, WALL_OFFSET, size=1)
        self._pose[YAW] = np.random.uniform(0, 2*np.pi, size=1)
        #print(self._pose)
        if (self.is_valid()):
	  not_val = False
  ...
  def compute_weight(self, front, front_left, front_right, left, right):
    ...
    if (val):
      #print(self._pose)
      theta = [0., np.pi / 4., -np.pi / 4., np.pi / 2., -np.pi / 2.]
      mu = [front, front_left, front_right, left, right]
      for i in range(5):

        x = self.ray_trace(theta[i])
        mu_ = min(mu[i], max(x, 3.5))
        f1 = (x - mu_)/sigma
        f2 = f1 ** 2
        f3 = -0.5 * f2
        f4 = np.exp(f3)
        f5 = f4/(sigma * np.sqrt(2*np.pi))
        self._weight *= f5
      #mu = front #min(front, front_left, front_right, left, right)
      #mu = min(3.5, mu)
      #print('mu is %s'%mu)
      #theta = np.sin(np.arccos(self._pose[YAW]))
      #x = self.ray_trace(0)#ray_trace(theta)
      #print('x is %s'%x)
      
      #print('f5 is %s'%f5)
      #self._weight = f5 #scipy.stats.norm.pdf(x, mu, sigma)
      self._weight += 1.e-300
    else:
      self._weight = 0;
