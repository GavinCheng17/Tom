...
class Motion:
  ...
  count = 0
  @classmethod
  def euler_(cls, current_pose, rt, dt):
    t = np.floor(rt)
    ...
    w = np.cos(tdt)
    theta = next_pose[YAW]
    next_pose[X] += dt*u*np.cos(theta)
    next_pose[Y] += dt*u*np.sin(theta)
    next_pose[YAW] += dt*w
    return next_pose
  @classmethod
  def evalRK4(cls, t, theta):
    u = 0.25
    k_x = u*np.cos(theta)
    k_y = u*np.sin(theta)
    k_yaw = np.cos(t)
  ...
  @classmethod
  def rk4_(cls, current_pose, rt, dt):
    t = np.floor(rt)
    next_pose = current_pose.copy()
    if cls.tt == t:
      cls.tt = t
    ...
    k1_x, k1_y, k1_yaw = Motion.evalRK4(tdt, current_pose[YAW])
    k2_x, k2_y, k2_yaw = Motion.evalRK4(tdt+(dt/2), current_pose[YAW]+(dt*k1_yaw/2))
    k3_x, k3_y, k3_yaw = Motion.evalRK4(tdt+(dt/2), current_pose[YAW]+(dt*k2_yaw/2))
    k4_x, k4_y, k4_yaw = Motion.evalRK4(tdt+dt, current_pose[YAW]+(dt*k3_yaw))
    next_pose[X] += dt*(k1_x+2*k2_x+2*k3_x+k4_x)/6
    next_pose[Y] += dt*(k1_y+2*k2_y+2*k3_y+k4_y)/6
    next_pose[YAW] += dt*(k1_yaw+2*k2_yaw+2*k3_yaw+k4_yaw)/6
...
def main(args):
  ...
  for color, dt in zip(colors, args.dt):
    ...
    for t in np.arange(0., 10., dt):
      robot_pose = Motion.euler_(robot_pose, t, dt)
