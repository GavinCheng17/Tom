...
def euler(current_pose, t, dt):
  ...
  w = np.cos(t)
  #w = np.cos(np.floor(t))
  theta = current_pose[YAW] #np.sin(t)
  #theta = np.sin(np.floor(t))
  next_pose[X] = current_pose[X] + dt*u*np.cos(theta)
  next_pose[Y] = current_pose[Y] + dt*u*np.sin(theta)
...
def rk4(current_pose, t, dt):
  next_pose = current_pose.copy()
  t = np.floor(t)
  k1_x, k1_y, k1_yaw = evalRK4(t, current_pose[YAW])
  k2_x, k2_y, k2_yaw = evalRK4(t+(dt/2), current_pose[YAW]+(dt*k1_yaw/2))
  k3_x, k3_y, k3_yaw = evalRK4(t+(dt/2), current_pose[YAW]+(dt*k2_yaw/2))
  k4_x, k4_y, k4_yaw = evalRK4(t+dt, current_pose[YAW]+(dt*k3_yaw))
  next_pose[X] = current_pose[X] + dt*(k1_x+2*k2_x+2*k3_x+k4_x)/6
  next_pose[Y] = current_pose[Y] + dt*(k1_y+2*k2_y+2*k3_y+k4_y)/6
  next_pose[YAW] = current_pose[YAW] + dt*(k1_yaw+2*k2_yaw+2*k3_yaw+k4_yaw)/6
